...
_AGENT_ORDER = ("Market", "Event", "Analysis", "Backtest", "Risk")
...
class DebateEngine:
    ...
    def _agent_speak(
        ...
        others_text = self._format_other_positions(agent_name, other_positions, round_number)
...
7. 回复使用结构化格式"""
        ...
        try:
            ...
            logger.warning("%s Agent LLM call failed in round %d: %s", agent_name, round_number, e)

        return self._fallback_turn(agent_name, prev_stance, contradiction_strings, evidence_text=ctx.evidence_text)

    def _format_other_positions(self, agent_name: str, positions: dict[str, dict[str, Any]], round_number: int) -> str:
        others = {k: v for k, v in positions.items() if k != agent_name}
        if not others:
            return "  (其他 Agent 尚未发言)"
        lines = []
        for name, stance in others.items():
            lines.append(f"  {name}: pressure={stance.get('pressure', 'N/A')}, confidence={stance.get('confidence', 'N/A')}")
            reasoning = stance.get("reasoning", "")
            if reasoning:
                lines.append(f"    理由: {reasoning[:200]}")
        return "\n".join(lines)

    def _moderate(self, round_number: int, turns: list[DebateTurn], contradictions: list[dict[str, Any]], evidence_board: list[EvidenceItem], previous_moderator_output: dict[str, Any] | None) -> ModeratorOutput:
        turns_text = "\n\n".join(self._format_turn_for_moderator(t) for t in turns)
        contradiction_descriptions = "\n".join(f"- [{c.get('id', '?')}] {c.get('description', str(c))}" for c in contradictions) if contradictions else "无"
...
注意: Risk Agent 提出 veto、硬风控失败、流动性问题或数据不足时, final_pressure 不得为 upgrade。"""

        system_msg = (
            "你是圆桌会议 Moderator, 根据所有 Agent 发言进行综合裁定。"
            "保持客观中立, 基于证据而非个人判断。"
            "Risk Agent 是一票否决角色: 硬风控、不可成交、数据质量缺口必须优先于收益叙事。"
        )

        try:
            result = self.llm.chat(messages=[("system", system_msg), ("human", prompt)], response_format=ModeratorOutput)
            if result:
    ...
    def _format_turn_for_moderator(self, turn: DebateTurn) -> str:
        ...
        return (f"--- {turn.agent_name} ---{changed}\n"
                f"立场: {turn.stance.pressure} (置信度: {turn.stance.confidence:.1f})\n"
                f"引用证据: [{evidence_ids}]\n"
                f"反驳: {rebuts}\n"
                f"理由: {turn.stance.reasoning[:500]}")

    def _check_convergence(self, round_number: int, turns: list[DebateTurn], moderator_output: ModeratorOutput) -> bool:
    ...
    def _default_turn(self, agent_name: str) -> DebateTurn:
        return DebateTurn(agent_name=agent_name, stance=AgentStance(pressure="neutral", confidence=0.3, evidence_ids=[], reasoning="数据不足, 默认中立。", changed_from_previous=False), rebuts=[], new_evidence=[])

    def _fallback_turn(self, agent_name: str, prev_stance: dict[str, Any] | None, contradictions: list[str], *, evidence_text: str = "") -> DebateTurn:
        if agent_name == "Risk":
            risk_text = f"{evidence_text} {' '.join(contradictions)}"
            lowered = risk_text.lower()
            if any(t.lower() in lowered for t in ("hard_veto", '"verdict": "HARD_VETO"', "涨停", "跌停", "veto")):
                return DebateTurn(agent_name=agent_name, stance=AgentStance(pressure="downgrade", confidence=0.9, evidence_ids=[], reasoning="LLM 不可用；Risk 证据中出现硬风控/veto/涨跌停不可成交信号，确定性降级。", changed_from_previous=False), rebuts=[], new_evidence=[])
            if any(t.lower() in lowered for t in ("soft_veto", '"verdict": "SOFT_VETO"', "数据不足", "缺失")):
                return DebateTurn(agent_name=agent_name, stance=AgentStance(pressure="downgrade", confidence=0.7, evidence_ids=[], reasoning="LLM 不可用；Risk 证据中出现软风控或数据质量缺口，按风控优先原则降级。", changed_from_previous=False), rebuts=[], new_evidence=[])
        if prev_stance:
            return DebateTurn(agent_name=agent_name, stance=AgentStance(pressure=prev_stance.get("pressure", "neutral"), confidence=prev_stance.get("confidence", 0.3) * 0.8, evidence_ids=prev_stance.get("evidence_ids", []), reasoning=f"LLM 不可用, 沿用上一轮立场: {prev_stance.get('reasoning', '')[:200]}", changed_from_previous=False))
        return DebateTurn(agent_name=agent_name, stance=AgentStance(pressure="neutral", confidence=0.3, evidence_ids=[], reasoning="LLM 不可用, 默认中立立场。", changed_from_previous=False))

    def _fallback_moderator(self, round_number: int, turns: list[DebateTurn], contradictions: list[dict[str, Any]]) -> ModeratorOutput:
        pressure_counts = {"upgrade": 0, "neutral": 0, "downgrade": 0}
        for turn in turns:
            pressure_counts[turn.stance.pressure] = pressure_counts.get(turn.stance.pressure, 0) + 1
        risk_downgrade = any(t.stance.pressure == "downgrade" for t in turns if t.agent_name == "Risk")
        if risk_downgrade:
            final_pressure = "downgrade"
        elif pressure_counts["downgrade"] > pressure_counts["upgrade"]:
            final_pressure = "downgrade"
        elif pressure_counts["upgrade"] > pressure_counts["downgrade"]:
            final_pressure = "upgrade"
        else:
            final_pressure = "neutral"
        return ModeratorOutput(round_number=round_number, final_pressure=final_pressure, unresolved_contradiction_ids=[c.get("id", "") for c in contradictions], consensus_items=["LLM 不可用, 默认共识"], dissent_items=[c.get("description", str(c)) for c in contradictions], converged=round_number >= 1, reasoning=f"LLM 不可用, 确定性回退", risk_focus=["Risk Agent 降级，保守处理"])

...
┌─────────────────────────────────────────────────────────────────┐
│                        入口层                                    │
│  CLI: --scan │ --ticker │ --data-agent │ --batch                 │
│  REPL: /scan │ /a │ /d │ /run │ /dates                          │
└──────────────────────────┬──────────────────────────────────────┘
                           │
                           ▼
┌─────────────────────────────────────────────────────────────────┐
│                   scan_and_collect()  一次搞定                    │
│                                                                  │
│  MarketScanner.scan()          MarketScanner.scan_and_collect()  │
│  ┌────────────────────┐       ┌────────────────────────────┐    │
│  │ 四通道扫描+评分     │       │ Phase 1: scan (同上)        │    │
│  │ ├ 热门板块成分股    │       │ Phase 2: 共享数据拉一次     │    │
│  │ ├ 涨停池           │       │  大盘/板块排行/ST/停牌/退市  │    │
│  │ ├ 北向十大成交      │       │ Phase 3: Top-N逐票数据      │    │
│  │ └ 龙虎榜           │       │  日线/资金流/新闻            │    │
│  │ → 排名候选列表      │       │ → ScanBundle                │    │
│  └────────────────────┘       └────────────────────────────┘    │
│                                                                  │
│  summarise_with_llm()  市场综述 (默认开启)                        │
│  → 市场情绪 / 板块分析 / 资金动向 / 候选逻辑                       │
│  → LLM不可用时降级为数据表格                                      │
│                                                                  │
│  缓存: scan_report_{trade_date}.md → 同日重复运行直接返回          │
└──────────────────────────┬──────────────────────────────────────┘
                           │
                           ▼
┌─────────────────────────────────────────────────────────────────┐
│                     DataAgent 流水线                              │
│                                                                  │
│  DataAgent.run(request, raw_data=预采集)                          │
│  ┌────────────┬────────────┬────────────┬──────────────────┐    │
│  │ 0.前置      │ 1.采集      │ 2.清洗      │ 3.分析             │    │
│  │ StockProfile│ route_to   │ DataCleaner│ FactorCalculator  │    │
│  │ Resolver    │ _vendor    │ + 板块标准化 │ + 资金/情绪/板块   │    │
│  │ + Planner   │ ↓          │ + 新闻统一  │ + LLM新闻筛选      │    │
│  │             │ 有raw_data │            │ + 风险检查         │    │
│  │             │ → 跳过     │            │ → tier1/tier2     │    │
│  └────────────┴────────────┴────────────┴──────────────────┘    │
│                                                                  │
│  持久化: 6层JSON (01_input → 02_raw → 03_cleaned →               │
│          04_analysis → 05_agent_payload → 06_final)              │
│  审计: DataManifest + route_trace + audit_trail                  │
└──────────────────────────┬──────────────────────────────────────┘
                           │  tier1_data + tier2_data
                           ▼
┌─────────────────────────────────────────────────────────────────┐
│                TradingSystem 多Agent 决策层                       │
│                                                                  │
│  LangGraph 状态机 (15个节点)                                      │
│  ┌──────────────────────────────────────────────────────────┐   │
│  │ Risk Check 1 → System Init → Memory Agent                │   │
│  │ → Market Agent → Event Agent → Analysis Agent            │   │
│  │ → Backtest Agent → Risk Check 2 → Round 2 Judge          │   │
│  │ → [Round 2 Debate 最多8轮] → Risk Check 3                │   │
│  │ → System Final Decision → Approval Agent → Report Agent  │   │
│  └──────────────────────────────────────────────────────────┘   │
│                                                                  │
│  若 tier1/tier2 已提供 → _load_data() 跳过, 零网络请求            │
│  否则 → _load_data() → DataAgent.run() 自己采集                   │
└──────────────────────────┬──────────────────────────────────────┘
                           │
                           ▼
┌─────────────────────────────────────────────────────────────────┐
│  输出: Markdown报告 + 审批记录 + 复盘字段                          │
│  写入: Memory Store (PostgreSQL + ChromaDB)                      │
└─────────────────────────────────────────────────────────────────┘
```
...
```
之前 (两遍网络请求):
  scan() → 候选列表 → for each: TradingSystem.analyze()
                                → _load_data() → DataAgent.run()
                                    → _collect_raw() 重新拉一遍

现在 (一遍):
  scan_and_collect() → ScanBundle (候选 + 全部raw数据)
    → LLM 市场综述 (一次)
    → for each:
        DataAgent.run(raw_data=预采集) → 跳过 _collect_raw
        → TradingSystem.analyze(tier1=..., tier2=...) → 跳过 _load_data
```
...
| 组件 | 文件 | 职责 |
|---|---|---|
| MarketScanner | data_agent/scanner.py | 四通道扫描 + 采集 + LLM综述 |
| ScanBundle | data_agent/scanner.py | 扫描结果 + 共享数据 + 逐票数据 |
| DataAgent | data_agent/data_agent.py | 采集→清洗→分析→持久化，6层审计 |
| vendor_router | data_agent/vendor_router.py | 多供应商故障转移 (akshare→baostock→eastmoney) |
| collector | data_agent/collector.py | 各供应商具体适配函数 |
| DataCleaner | data_agent/cleaner.py | 日线清洗、涨跌停检测 |
| FactorCalculator | data_agent/factors.py | 6因子计算 (质量/成长/估值/动量/波动率/流动性) |
| TradingSystem | graph/workflow.py | LangGraph 15节点多Agent决策 |
| MarketAgent | agents/market_agent.py | LLM市场情绪分析 |
| EventAgent | agents/event_agent.py | LLM事件驱动分析 |
| AnalysisAgent | agents/analysis_agent.py | LLM因子综合分析 |
| BacktestAgent | agents/backtest_agent.py | LLM历史回测审查 |
| SystemAgent | agents/system_agent.py | 控场+裁定 |
| MemoryAgent | agents/memory_agent.py | 历史经验召回+写入 |
...
### 4.1 DataAgent — 数据底座（当前实现）

DataAgent 是系统的数据底座，不是 Agent（不做LLM推理决策），而是一条确定性的6阶段数据流水线。负责把行情、资金、板块、新闻、风险状态加工成 tier1/tier2 统一格式，让后续 Agent 不直接面对脏数据。

**当前实现位置：** `src/advanced_trading_agent/data_agent/`

**6 阶段流水线：**

```
DataAgentRequest (Pydantic validated)
  ticker, trade_date, include_*, use_react_planner, ...
      │
      ▼
┌──────────────────────────────────────────────────┐
│ Stage 0: 前置                                     │
│ ├ StockProfileResolver.resolve(ticker)           │
│ │   → akshare查询 或 内置静态表                   │
│ │   → 公司名 / 行业关键词 / 别名 / 置信度          │
│ ├ [可选] DataAgentPlanner.plan()                 │
│ │   → LLM ReAct: 观察→思考→行动                  │
│ └ 创建 run_dir + DataManifest                    │
│ 输出 → 00_planner/plan.json                      │
│ 输出 → 01_input/request.json                     │
└──────────────────────────────────────────────────┘
      │
      ▼
┌──────────────────────────────────────────────────┐
│ Stage 1: _collect_raw  ← 或 raw_data参数跳过     │
│                                                    │
│ vendor_router 做多供应商故障转移:                    │
│ akshare → baostock → sina/eastmoney               │
│                                                    │
│ ├ get_daily(ticker)          → 个股日线 (qfq复权) │
│ ├ get_daily(“000001.SH”)     → 上证指数日线       │
│ ├ get_sector(top_n=20)       → 板块排行           │
│ ├ get_capital_flow(ticker)   → 个股资金流向       │
│ ├ get_news(ticker, keyword)  → 新闻+公告          │
│ │   └ _enrich_news_full_text → URL抓正文+清洗     │
│ └ 风险三件套 (all-or-nothing):                    │
│    ├ get_st_status()    → ST风险名单              │
│    ├ get_suspended()    → 停牌名单                │
│    └ get_delisting()    → 退市风险名单            │
│                                                    │
│ 每步记录: route_trace (供应商/耗时/状态)            │
│         + manifest (字段可用性清单)                 │
│ 输出 → 02_raw/raw_data.json                       │
└──────────────────────────────────────────────────┘
      │
      ▼
┌──────────────────────────────────────────────────┐
│ Stage 2: _clean                                   │
│                                                    │
│ ├ DataCleaner.clean_daily(raw)                    │
│ │   → 列名标准化 / 空值处理 / trade_date排序       │
│ ├ DataCleaner.detect_limit_up_down(df)            │
│ │   → 标记 is_limit_up / is_limit_down            │
│ ├ _clean_sector_context(records)                  │
│ │   → sector_name/change_pct/strength 统一        │
│ ├ 大盘日线 → DataFrame清洗                         │
│ └ _clean_news(records)                            │
│    → title/summary/full_text/evidence_text 统一   │
│    → 去HTML/去噪/去重                              │
│                                                    │
│ 输出 → 03_cleaned/cleaned_data.json                │
└──────────────────────────────────────────────────┘
      │
      ▼
┌──────────────────────────────────────────────────┐
│ Stage 3: _analyze                                 │
│                                                    │
│ ├ FactorCalculator.run_all(daily_df)              │
│ │   → ROE / 营收增长 / PE分位 / PB分位             │
│ │   → 动量20日 / 波动率 / Amihud流动性 / 综合评分  │
│ │                                                │
│ ├ _summarize_market(market_df)                   │
│ │   → 指数收盘价 / 涨跌幅                          │
│ │   → 情绪档位: ≤-2%低迷 / ≥+2%温热 / 正常       │
│ │                                                │
│ ├ _summarize_sector_context(records, request)    │
│ │   → 板块匹配: 关键词→行业名 启发式对齐           │
│ │   → 匹配策略: keyword_match / top_rank_fallback │
│ │                                                │
│ ├ _summarize_capital(capital_flow)               │
│ │   → 主力净流入 / 散户净流入                      │
│ │   → 资金确认/背离 判断                           │
│ │                                                │
│ ├ _summarize_risk(risk_data, latest)             │
│ │   → ST/停牌/退市 命中检查                        │
│ │   → 当日涨跌停状态                               │
│ │                                                │
│ ├ _filter_news_with_llm(news, request)           │
│ │   → LLM: 判断相关度/方向(正面/负面/中性)/置信度   │
│ │   → LLM不可用: 关键词兜底                         │
│ │   → 全部被LLM过滤: guardrail补回                 │
│ │                                                │
│ ├ _build_event_records(news, request)            │
│ │   → event_id / event_type / direction           │
│ │   → transmission_path / evidence_level          │
│ │   → pricing_status / chain_quality              │
│ │                                                │
│ ├ _build_daily_consistency_report(daily, trace)  │
│ │   → 多源交叉校验: close/pct_chg/volume/amount   │
│ │   → 冲突/一致/单源 判定                          │
│ │                                                │
│ └ _build_agent_payload(...)                      │
│    ├ tier1_data: market / sentiment / capital    │
│    │              / risk / sector                 │
│    └ tier2_data: price_data / factors / events   │
│                   / sector_context / data_quality │
│                                                    │
│ 输出 → 04_analysis/analysis_data.json              │
│ 输出 → 04_analysis/news_events.json                │
│ 输出 → 05_agent_payload/agent_payload.json         │
└──────────────────────────────────────────────────┘
      │
      ▼
┌──────────────────────────────────────────────────┐
│ Stage 4: 汇总 & 持久化                             │
│ ├ 06_final/response.json  完整流水线快照          │
│ ├ DataManifest.save()     字段可用性清单           │
│ └ collection_summary      采集健康报告             │
│                                                    │
│ 返回 DataAgentRun:                                │
│ ├ artifacts: 6层产物路径+记录数                    │
│ ├ collection_summary: 成功/失败/空数据分类统计     │
│ ├ audit_trail: 每步审计日志                       │
│ └ errors: 各阶段错误列表                           │
└──────────────────────────────────────────────────┘
```

**与 MarketScanner 的协作模式：**

```
独立模式: DataAgent.run(request)
  → _collect_raw 自己拉数据 → clean → analyze → persist

整合模式: DataAgent.run(request, raw_data=预采集)
  → _adopt_raw 接管预采集数据 → 跳过_collect_raw
  → clean → analyze → persist (后续完全一样)
```

**vendor_router 多供应商故障转移：**

```
route_to_vendor(“get_daily”, code=”000001.SZ”)
  → try akshare: get_daily_akshare()
  → fail → try baostock: get_daily_baostock()
  → fail → NoMarketDataError (上游处理降级)
...
| 方法 | 供应商 | 用途 |
|---|---|---|
| get_daily | akshare, baostock | 日线OHLCV |
| get_capital_flow | akshare | 个股资金流向 |
| get_news | akshare, sina | 新闻+公告 |
| get_sector | akshare, eastmoney | 板块排行 |
| get_sector_constituents | akshare | 板块成分股 |
| get_financial | akshare | 财务指标 |
| get_northbound_flow | akshare | 北向资金净流入 |
| get_northbound_top10 | akshare | 北向十大成交 |
| get_limit_up_tiers | akshare | 涨停梯队(含个股明细) |
| get_dragon_tiger | akshare | 龙虎榜 |
| get_margin | akshare | 融资融券 |
| get_st_status | akshare | ST名单 |
| get_suspended | akshare, baostock | 停牌名单 |
| get_delisting | akshare | 退市风险名单 |
| get_factors | akshare, baostock | 因子计算(computed) |
